...
public class AccountInfoOperationServiceImpl implements AccountInfoOperationService {
    ...
    public boolean checkParamExist(String username, String email, String phone) {
        if (!StringUtils.isEmpty(username)) {
            Account account = accountAuthOperationService.getAccountTkService().queryOne(new Account(username));
            if (account != null) {
                return true;
            }
        }
        ...
        if (!StringUtils.isEmpty(email)) {
            Account account = accountAuthOperationService.getAccountTkService().queryOne(new Account(email));
            if (account != null) {
                return true;
            }
        }
        ...
        if (!StringUtils.isEmpty(phone)) {
            ...
            account = accountAuthOperationService.getAccountTkService().queryOne(account);
            return account != null;
        }
    ...
    public int getAccountMaxAuthorityRoleLevel(Long id) {
        Account account = accountAuthOperationService.getAccountTkService().queryById(id);
        ...
        List<Role> roles = accountAuthOperationService.getRoleTkService().queryRolesByNames(Arrays.asList(roleArrays));
    ...
    public boolean registryAccount(UserDTO userDTO, List<Role> roles) {
        ...
        if (!accountAuthOperationService.getAccountTkService().insert(account)) {
            return false;
        }
        ...
        AssertUtil.isTrue(accountAuthOperationService.getAccountProfileTkService().insert(accountProfile), "Failed execute to insert account profile, data: " + JsonUtil.toJson(accountProfile));

        // insert account oauth2 client
        SysOauthClient sysOauthClient = buildAccountOauthClient(account);
        AssertUtil.isTrue(accountAuthOperationService.getAccountOauthClientTkService().insert(sysOauthClient),
                "Failed execute to insert to oauth2 client, data: " + JsonUtil.toJson(sysOauthClient));
    ...
    private SysOauthClient buildAccountOauthClient(Account account) {
        return new SysOauthClient(account.getUsername(), account.getPassword());
    }
    ...
    public void editAccount(UserDTO userDTO, List<Role> roles, Account account, List<Role> oldRoles) {
        ...
        AssertUtil.isTrue(accountAuthOperationService.getAccountTkService().update(account), INVALID_UPLOAD_FILE.message);
        ...
        //update oauth client
        SysOauthClient client = accountAuthOperationService.getAccountOauthClientTkService().queryById(account.getUsername());
        AssertUtil.notNull(client, "Oauth client should not be null.");
        setOauthClient(client, account);
        AssertUtil.isTrue(accountAuthOperationService.getAccountOauthClientTkService().update(client), "Failed execute to update oauth client.");

        //delete cache
        accountBaseInfoCacheService.invalid(account.getId());
    }

    private void setOauthClient(SysOauthClient client, Account account) {
    ...
    public void deleteUser(Account account) {
        ...
        AssertUtil.isTrue(accountAuthOperationService.getAccountTkService().update(account), "Failed execute to update account.");
        SysOauthClient sysOauthClient = accountAuthOperationService.getAccountOauthClientTkService().queryById(account.getId());
        if (sysOauthClient != null) {
            sysOauthClient.setStatus(false);
            AssertUtil.isTrue(accountAuthOperationService.getAccountOauthClientTkService().update(sysOauthClient), "Failed execute to update oauth client.");
        }
    }
