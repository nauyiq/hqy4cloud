...
public class AdminMenuController {
    ...
    public R<List<AdminMenuInfoVO>> getMenu(HttpServletRequest request) {
        return requestService.getAdminMenu(AuthUtils.getCurrentUserId());
    }
    ...
    @GetMapping("/menu/tree/self")
    public R<List<Integer>> getMenuPermissionsIdByAccountId() {
        Long currentUserId = AuthUtils.getCurrentUserId();
        return requestService.getMenuPermissionsIdByUserId(currentUserId);
    }

    @GetMapping("/menu/tree")
    public R<List<AdminTreeMenuVO>> getTreeMenu(HttpServletRequest request, Boolean status) {
        Long currentUserId = AuthUtils.getCurrentUserId();
        return requestService.getAdminTreeMenu(currentUserId, status);
    }
