 ...
 * @see RedisTemplateCommonOperations
 * @author qiyuan.hong
 * @version 1.0
 * @date 2022/10/26 15:55
 */
public abstract class DefaultRedisOperations implements RedisTemplateCommonOperations {
   private static final Logger log = LoggerFactory.getLogger(DefaultRedisOperations.class);

    private final RedisTemplate<String, Object> redisTemplate;

    public DefaultRedisOperations(RedisTemplate<String, Object> redisTemplate) {
    ...
    public final Set<String> scan(String matchKey, int matchCount) {
        try {
            return redisTemplate.execute((RedisCallback<Set<String>>) connection -> {
                ...
                Cursor<byte[]> cursor = connection.scan(new ScanOptions.ScanOptionsBuilder().match("*" + matchKey + "*").count(matchCount).build());
