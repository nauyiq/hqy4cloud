...
/**
    ...
    public void postRead(Object context, String methodName, Object[] args) throws TException {
        ...
        try {
            //读取动态的拓展的参数.
            ThriftRequestPram thriftRequestPram = (ThriftRequestPram) args[args.length - 1];
            ((ThriftContext)context).setParam(thriftRequestPram);
            //移除 ThriftMethodHandler中动态添加的参数.
            ArgsUtil.reduceTailArg(args);
            boolean bind = getInjectTransactionXidAndBinding(thriftRequestPram, methodName);
    ...
    private boolean getInjectTransactionXidAndBinding(ThriftRequestPram thriftRequestPram, String methodName) {
        if (CommonSwitcher.ENABLE_PROPAGATE_GLOBAL_TRANSACTION.isOn()) {
            String rpcXid = thriftRequestPram.xid;
            ...
            if (StringUtils.isBlank(xid) && StringUtils.isNotBlank(rpcXid) && TransactionContext.isTransactional(methodName)) {
                ...
                log.warn("@@@ Checking method : {} need binding rpcXid, but rpcXid is empty. exParam:{}", methodName, JsonUtil.toJson(thriftRequestPram));
            }
        } else {
