package com.corundumstudio.socketio.ex.listener;
...
public class DefaultAuthorizationListener implements AuthorizationListenerAdaptor {
    ...
    public boolean isAuthorized(HandshakeData data) {
        ...
        if (StringUtils.isBlank(accessToken) || JWT.getInstance().isExpired(accessToken)) {
            //安全校验策略不通过
            ...
            if (StringUtils.isEmpty(data.getBizId())) {
                //业务增强 设置业务id bizId
                SocketProjectContext context = JWT.getInstance().decrypt(accessToken, SocketProjectContext.class);
